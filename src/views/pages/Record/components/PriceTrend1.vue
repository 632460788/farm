<template>
  <div id="root">
    ...
    <div style="display:flex; flex-direction: row; align-items:center; justify-content: start">
      <span style="font-size:50px">市场价格</span>&nbsp;&nbsp;
    ...
    <div style="display:flex;justify-content:space-around;">
      <div style="display:flex;width:32.5%;border-style: solid; border-width: 5px;" :class="{rise: showData.todayPrice > showData.yestodayPrice, down: showData.todayPrice < showData.yestodayPrice}">
        ...
        <div class="gravity_center" style="flex-direction:column;font-size:32px;flex:1">
          ...
          <span v-if="showData.todayPrice === '暂无' || showData.yestodayPrice === '暂无'"> -- </span>
          <span v-else-if="showData.todayPrice > showData.yestodayPrice" >涨幅: {{ ((showData.todayPrice - showData.yestodayPrice) / showData.yestodayPrice * 100).toFixed(2)}}%</span>
          <span v-else-if="showData.todayPrice === showData.yestodayPrice"> 价格无变动 </span>
          <span v-else>跌幅: {{((showData.todayPrice - showData.yestodayPrice) / showData.yestodayPrice * 100).toFixed(2)}} %</span>
        </div>
      </div>
  ...
  export default {
    ...
    methods:{
      ...
      getAreaPrice:async function () {
        let param = {time: 365};
        await this.$api.price.findMarketPriceByTime(param).then((res) => {
          ...
          if (res.code === 200)
          {
            ...
            let tempPriceDict = {}
            let returnPriceDict = {}
            if (res.data.marketPrice.length !== 0) {
              //tempPriceInfo  [{"area":...,"date":...,"price":...},{}]
              let tempPriceInfo = res.data.marketPrice
              for (let i = 0; i < tempPriceInfo.length ; i++) {
                let date = tempPriceInfo[i]["date"]
                let area = tempPriceInfo[i]["area"] +'-'+tempPriceInfo[i]["spec"]
                let price = tempPriceInfo[i]["price"]
                if(Object.keys(tempPriceDict).indexOf(area) === -1){
                  tempPriceDict[area] = {}
                  tempPriceDict[area][date] = price
                }
                else {
                  tempPriceDict[area][date] = price
                }
