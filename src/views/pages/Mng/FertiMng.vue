<template>
  <div class="page-container">
    ...
    <!--新增编辑界面-->
    <!--     TODO 可优化新增编辑界面-->
    <el-dialog :title="operation?'新增施肥记录':'编   辑'" width="40%" :visible.sync="dialogVisible" :close-on-click-modal="false">
      <el-form :model="dataForm" label-width="120px" :rules="dataFormRules" ref="dataForm" :size="size"
        ...
        <el-form-item label="肥料名称" prop="kind">
          <el-input v-model="dataForm.kind" auto-complete="off" ></el-input>
        </el-form-item>
        <el-form-item label="施肥量(千克)" prop="quantity">
          <el-input v-model="dataForm.quantity" auto-complete="off" ></el-input>
        </el-form-item>
    ...
    data() {
      return {
        ...
        dataFormRules: {
          ...
          kind: [
            {required: true, message: '请输入肥料名称', trigger: 'blur'}
          ],
          quantity:
          //   [
          //   {required: true, message: '请输入施肥的量', trigger: 'blur'}
          // ]
            [
              {required: true, message: '请输入施肥量' , trigger: 'blur'},
              {
                pattern: /^(-?\d+)(\.\d+)?$/,  //正则
                message: '请验证输入格式',
              }
    ...
    methods: {
      ...
      findPageByDate: function(){
        ...
        if ((this.filters.LotID === '' || this.isNULL(this.filters.LotID)) && (this.filters.start === '' || this.isNULL(this.filters.start)) && (this.filters.end === '' || this.isNULL(this.filters.end))){
          ...
          this.$api.fertilization.findPageByDate(params).then((res) => {
      ...
      findPage: function (data) {
        ...
        this.$api.fertilization.findPage(null).then((res) => {
      ...
      handleDelete: function (data) {
        ...
        console.log(param)
        try{
          this.$api.fertilization.batchDelete(param).then(data !== null ? data.callback : '')
        }
        catch (e) {
        ...
        this.$refs.dataForm.validate((valid) => {
          if (valid) {
            this.$confirm('确认提交吗？', '提示', {}).then(() => {
              ...
              this.$api.fertilization.save(params).then((res) => {
      ...
      initColumns: function () {
        this.columns = [
          ...
          {prop: "kind", label: "施肥种类", minWidth: 120},
          {prop: "quantity", label: "施肥数量(千克)", minWidth: 120},
